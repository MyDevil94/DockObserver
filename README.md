...
DockObserver is a webapp for managing and updating your docker containers. It is built using Go.
...
In order to properly update each stack, DockObserver discovers the stack's docker-compose file and runs `docker compose -f <config_file> up --pull always` for updates.
DockObserver also looks for a `.env` file in the same directory as the compose file.
...
To work around this, DockObserver caches the results of the API calls and only queries Dockerhub when the cache expires.
...
DockObserver has a `/api/stats` route that returns some basic stats about the containers it is managing.  
...
- DockObserver:
    widget:
      type: customapi
      url: http://dockobserver:3000/api/stats
      mappings:
        - field: numOfStacks
          label: Stacks
          format: number
